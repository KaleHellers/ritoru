...
void forgot_pass() {
	system("cls");
	int choice;
	cout << "\n\n\t\t            Меню восcтановления пароля         " << endl;
	cout << "\t\t        ---------------------------------\n" << endl;
	cout << "\t\t\tЗабыли?! Сейчас мы вам поможем" << endl;
	cout << "\t\t\t[Нажмите 1]Восстановить пароль\n" << "\t\t\t[Нажмите 2]Меню" << endl;
	cout << "\t\t\tВаш выбор:";
	while (!(cin >> choice) || (cin.peek() != '\n'))
	{
		cin.clear();
...
void password_recovery() {
	...
	if (success) {
		...
		cout << "\t\t\tПользователся не зарегистрирован" << endl;
	}
	file_pass.close();
	system("pause");
	system("cls");
...
void registration() {
	int choice;
	string reg_name, reg_pass, check_username, check_password, name, surname, otc, age, country, instrument, place, number;
	system("cls");
	...
	file_position.open("info.txt", ofstream::app);

	file_position << reg_name << ' ' << surname << ' ' << name << ' ' << otc << ' ' << age << ' ' << country << ' ' << number << ' ' << instrument << endl;
	file_position.close();
